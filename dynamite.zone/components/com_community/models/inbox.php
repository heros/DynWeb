...
//			$sql .= ' WHERE (a.'.$db->quoteName('to').' = '.$db->Quote($to) . ' OR a.'.$db->quoteName('msg_from').' = '.$db->Quote($to) . ')';
            ...
            $sql .= (strlen($strId)) ? " b.`id` IN (".$strId.") AND  " : ' 0 AND ';
            ...
            $sql .= (strlen($strId)) ? " b.`id` IN ('.$strId.') AND " : "" ;
    ...
    public function &getRecepientMessage($id){
        ...
        foreach($result as $recipient) {
            ...
            if($getBlockStatus->isUserBlocked($recipient->msg_from, 'inbox')) continue;
            if($getBlockStatus->isUserBlocked($recipient->to, 'inbox')) continue;
            $return[] = $recipient;
        }
