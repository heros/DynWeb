...
class CommunityModelPhotos extends JCCModel implements CLimitsInterface {
    ...
    public function getAlbumPhotoCount($where = '', $limit = NULL, $limitstart = NULL, $orderBy = ' ORDER BY a.`created` DESC') {
    ...
    public function getProfileAlbums($userId = '', $pagination = false, $doubleLimit = false){
        $albums = $this->_getAlbums($userId, PHOTOS_PROFILE_TYPE, $pagination, $doubleLimit, '', '', '', array('profile.avatar')); // exclude profile avatar album to be listed
        return $albums;
    }
    ...
    public function updateAlbumName($albumid, $name) {
        $db = $this->getDBO();
        $query = 'UPDATE #__community_photos_albums SET '.$db->quoteName('name').'=' . $db->Quote($name) . 'WHERE `id`=' . $db->Quote($albumid);
        $db->setQuery($query);
        $db->query();
