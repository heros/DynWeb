...
Tagging.prototype.initialize = function() {
    ...
    this.$textarea
        .off( 'focus.'  + namespace )
        .on( 'focus.'   + namespace, $.proxy( this.inputOnKeydown, this ) )
        .on( 'click.'   + namespace, $.proxy( this.inputOnKeydown, this ) )
        .on( 'keydown.' + namespace, $.proxy( this.inputOnKeydown, this ) )
        .on( 'keyup.'   + namespace, $.proxy( this.inputOnKeyup, this ) )
        .on( 'input.'   + namespace, $.proxy( this.inputOnInput, this ) )
        .on( 'blur.'    + namespace, $.proxy( this.inputOnBlur, this ) );

    this.$dropdown
        .on( 'mouseenter.' + namespace, cssDropdownItem, $.proxy( this.dropdownOnMouseEnter, this ) )
        .on( 'mousedown.'  + namespace, cssDropdownItem, $.proxy( this.dropdownOnMouseDown, this ) )
        .on( 'mouseup.'    + namespace, cssDropdownItem, $.proxy( this.dropdownOnMouseUp, this ) );
            ...
            if ( tag.start >= this.prevSelStart ) {
                ...
                // Cursor position is inside a tag.
                    ...
                    // Backspace.
                        ...
                        rMatch = new RegExp( '^(.{' + tag.start + '})(.{' + ( tag.length + delta ) + '})' );
                        rReplace = '$1' + name;
...
Tagging.prototype.beautifierUpdate = joms._.debounce(function( value, tags ) {
    ...
    if ( tags.length ) {
        ...
        for ( i = 0; i < tags.length; i++ ) {
            tag = tags[i];
            rMatch += '(.{' + ( tag.start - start ) + '})(.{' + tag.length + '})';
...
Tagging.prototype.dropdownSelectItem = function( e ) {
    var el       = e ? $( e.target ) : this.dropdownSelectedItem,
        id       = el.data('id'),
        name     = el.data('name'),
    ...
    re = new RegExp( '^(.{' + index + '}).{' + ( cpos - index ) + '}' );
