...
function initialize() {
    ...
    if ( !window.joms_singleactivity && !window.joms_filter_hashtag ) {
...
function getEdgeStreamId( edge ) {
    ...
    stream = container.find('.joms-stream').not('.joms-stream--adagency')[ edge ]();
    if ( !stream.length )
        return 0;
...
function injectAdAgencyItem() {
    var ads, config, after, every, counter, isAfter, pageMap;
