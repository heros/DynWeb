...
class CommunityModelUsers extends JModelLegacy {
    ...
    public function getAllUsers($useLimit = true, $useSearch = true) {
        if (empty($this->_data)) {
            ...
            $useSearch = JRequest::getString('usesearch',true);
            ...
            switch ($usertype) {
                case 'jomsocial':
                    ...
                    break;
                case 'facebook':
                    $joinQuery = 'INNER JOIN ' . $db->quoteName('#__community_connect_users') . ' AS c '
                            . 'ON a.id=c.userid LEFT JOIN '
            ...
            if($usertype == 'facebook'){
                ...
                $searchQuery .= ' AND (c.type=' . $db->Quote('facebook') . ') ';
            }elseif ($usertype == 'jomsocial') {
                $searchQuery .='AND a.`id` NOT IN(  SELECT '.$db->quoteName('userid').' FROM '. $db->quoteName('#__community_connect_users').' )';
            }
